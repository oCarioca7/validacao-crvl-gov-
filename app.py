...
<div class="admin-box">
    ...
    <form id="formAdmin">
        ...
        <div class="form-group"><label>Chassi Completo (A IA mascarará automaticamente)</label><input type="text" id="chassi" required></div>
...
<script>
    async function gerarAutenticacao() {
        ...
        if(response.ok) {
            ...
            // Renderiza o QR Code via API pública geradora de QR Codes rápidos
...
<head>
    ...
    <style>
        ...
        .field-value { font-size: 14px; color: #14171a; font-weight: 700; text-align: right; font-family: inherit; }
...
@app.route('/')@app.route('/admin')def admin():# Rota do painel administrativoreturn render_template_string(HTML_ADMIN)@app.route('/api/criar', methods=['POST'])def api_criar():dados = request.jsonif not dados:return jsonify({"error": "Dados inválidos"}), 400# Cria um ID de consulta únicoid_consulta = str(uuid.uuid4())[:8]# Processamento e máscara do chassi por segurança e conformidade normativachassi_original = dados.get('chassi', '').strip()ultimos_digitos = chassi_original[-4:] if len(chassi_original) >= 4 else chassi_originalchassi_mascarado = f"***{ultimos_digitos.upper()}"# Fuso horário oficial de Brasíliafuso_brasilia = timezone(timedelta(hours=-3))data_hora_atual = datetime.now(fuso_brasilia).strftime("%d/%m/%Y %H:%M:%S")# Armazena o registro completo no banco de dados temporárioBANCO_DADOS[id_consulta] = {"nome": dados.get('nome', '').upper(),"doc_proprietario": dados.get('doc_proprietario', '') or "---","placa": dados.get('placa', '').upper(),"renavam": dados.get('renavam', ''),"chassi_mascarado": chassi_mascarado,"modelo": dados.get('modelo', '').upper(),"ano": dados.get('ano', ''),"cla_seguranca": dados.get('cla_seguranca', '') or "---","crv_numero": dados.get('crv_numero', '') or "---","data_hora": data_hora_atual}# Retorna o link absoluto de validação do policialurl_completa = request.host_url.rstrip('/') + url_for('validar_policial', id_consulta=id_consulta)return jsonify({"id": id_consulta, "url_validacao": url_completa})@app.route('/validar/<id_consulta>')def validar_policial(id_consulta):# Rota que o Policial/Cliente verá ao abrir o link do QR Coderegistro = BANCO_DADOS.get(id_consulta)if not registro:return "Erro 404: Registro de autenticação não encontrado na base do SENATRAN.", 404return render_template_string(HTML_POLICIAL, dados=registro)if name == "main":porta = int(os.environ.get("PORT", 5000))app.run(host="0.0.0.0", port=porta)
